...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // If we have movie data and we're adding to favorites, ensure it's in our database
      ...
      if (action === "add") {
        ...
        if (!user.favorites.includes(movieId)) {
          await usersCollection.updateOne({ _id: new ObjectId(user._id) }, { $push: { favorites: movieId } })
        }
      } else {
        // Remove movie from favorites
        await usersCollection.updateOne({ _id: new ObjectId(user._id) }, { $pull: { favorites: movieId } })
      }
